...
void redis_callback(WFRedisTask *task) {
    ...
    series_context* context = (series_context*)series_of(task)->get_context();
    RedisValue value;
...
void http_callback(WFHttpTask *task) {
    ...
    WFRedisTask *redis_task = WFTaskFactory::create_redis_task(context->redis_url, RETRY_MAX, redis_callback);

    std::string value((char*)body, body_len);
    redis_task->get_req()->set_request("SET", {context->redis_url, value});
...
int main(int argc, char* argv[]) {
    ...
    if (strncasecmp(argv[2], "redis://", 8) != 0 && strncasecmp(argv[1], "rediss://", 9) != 0) {
        context.redis_url = "redis://" + context.redis_url;
    }

    http_task = WFTaskFactory::create_http_task(context.http_url, REDIRECT_MAX, RETRY_MAX, http_callback);
